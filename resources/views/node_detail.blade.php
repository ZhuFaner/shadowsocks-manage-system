...
@section('content')
    <script type="text/javascript">
        $(document).ready(function () {
            $.get("{{ url("rank?type=day") }}" + "&node={{$node->node_address}}",function (data) {
                var json = JSON.parse(data);
                var title = "最近一天流量使用排名（单位：KB）";
                drawSheet(title, json.labels, json.flows,json.bg_colors)
            });
            $("#btn_last_hour").click(function () {
                $.get("{{ url('rank?type=hour') }}" + "&node={{$node->node_address}}",function (data) {
                    var json = JSON.parse(data);
            ...
            $("#btn_last_week").click(function () {
                $.get("{{ url('rank?type=week') }}" + "&node={{$node->node_address}}",function (data) {
                    var json = JSON.parse(data);
        ...
        <div class="row"> 
            <div class="col-md-10 col-md-offset-1">
                <div class="panel panel-default">
                    <div class="panel-heading">{{empty($node->name) ? $node->node_address : $node->name}}</div>
